...
最大不能超过 50；若有效候选少于最少标签数，下限自动缩减到候选数量。自然语言
默认生成 1–3 句，范围必须在 1–10 句内。标签或句子数量不合规时会进行有限次数
重试。LLM 少选标签时会从随机候选补足下限，超出上限时会截断。标签只会从
CSV 召回候选集中选择；无有效候选时只输出自然语言描述。
每个已开启且召回到相关候选的分类会至少保留一个标签；若相关分类数量超过
`max_tags`，随机选择能够覆盖的分类。指定 seed 时随机结果可复现，`-1` 时每次
重新随机。
