...
最大不能超过 50。自然语言默认生成 1–3 句，范围必须在 1–10 句内。程序先从每个
已开启且有数据的分类中随机加入一个保底标签，再随机补充至最多 100 个候选，并在
标签数量范围内随机确定最终数量。LLM 同时接收完整用户输入和候选池，不会先对输入
进行拆词检索。程序会删除 LLM 返回的重复和池外标签，并在传给描述 LLM 前补齐缺失
分类；超过目标数量时只随机丢弃分类内的额外标签，不会移除该分类的最后一个标签。
