...
无分类路径的 141,561 个标签不会进入候选池，也不会发送给 LLM。其余标签由
Generator 上的开关控制：
...
| General: Image composition and style | 1,109 | 开启 |
...
最大不能超过 50。自然语言默认生成 1–3 句，范围必须在 1–10 句内。程序从所有
已开启分类中随机建立最多 100 个标签的候选池，并在标签数量范围内随机确定最终
数量。LLM 同时接收完整用户输入和候选池，不会先对输入进行拆词检索。程序会删除
LLM 返回的重复和池外标签；超过目标数量时随机丢弃，不足时从候选池随机补足。
`min_tags` 和 `max_tags` 限制最终输出的标签总数，而不是单个分类的数量。指定 seed
时候选池、目标数量和修正结果均可复现，`-1` 时每次重新随机。所有已开启分类仍
无法提供 `min_tags` 个候选时会报错；LLM 返回的结构无法解析时会进行有限次数重试。
